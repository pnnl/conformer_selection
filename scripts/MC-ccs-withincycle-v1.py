...
def ccs_converge(cedf, nlst, iter=1000):
    """
    Args:
...
#      energy (list, float): array of energy values mapping exactly to the ccs array
      cedf (pandas.DataFrame): ccs and energy dataframe
...
#      List of standard deviations for each average
    ...
    for j, n in enumerate(nlst):
        ...
        for i in range(0, iter):
            idx = []
            cyc = np.random.choice(range(1, 1001), n, replace=False)#range(len(ccs)), n, replace=False)

            #this needs to change, this is probably what's taking so long
            for c in cyc:
                idx.extend(cedf.index[cedf['cycle'] == c].values)
            ccs_sample = cedf['ccs'][idx].values
            energy_sample = cedf['dft_energy'][idx].values
...
def file_handle(molid, dfpath, writepath='', ssize=50, iter=1000):
    ...
    # Build sample array for per cycle
    nlst = [x for x in range(1, 1001)]

    wdf = ccs_converge(df, nlst, iter)
