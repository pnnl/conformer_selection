...
# def mSim(mtrx, m=1):
#     """Finds the most similar set of size m using Monte Carlo (MC) simulation.  
#     Args:
#       mtrx (np.array): contains pairwise rmsd matrix where each row and colum correponds to a conformation.
#       m (int): Conformer set size for comparing similarity, i.e. the n most similar geometries
#     Returns:
#       indices of the m most similar conformers
#     """
#     # Sum all pw rmsd for each conformer.
#      # Conformers with smallest sums are the most similar geometries.
#     sums = mtrx.sum(axis=1)
#     indices = nansum_argsort(sums)
...
#     # Initialize arrays for capturing combo parameters, CCS results, and time
#     totaltimes = []       
#     combos = []
#     bw = []
#     le = []
...
#     df.sort_values(by='50k SDS Rank')
...
#    print('For BET, SDS, and random')
    ...
    num_cells = int(2 + 2*len(BET2_params)*(len(SDS2_params_init) + len(R2_params_init) + 2)) #I changed 2+3* to 1+2*  ***
...
#         idx = random.choice(subdf_s.index, rp, replace=False)
#         subdf_r = subdf_s.loc[idx].copy()
#         sub_r_len = len(subdf_r)
        idx = np.random.choice(df.index, num_confs, replace=False)
        subdf = df.loc[idx].copy()
        sub_len = len(subdf)


        # First, do without 'calculating' DFT energies on AMBER structures.
        ## Placeholder for bp2 param and 'other' param for tracking which methods were used
        #bp2 = np.nan
        #op2 = np.nan
        
        # # DFT GO
        # allnandrop_len = len(subdf.dropna()) # I feel like this could slow things down. If faster way, do it.
        #                                         # like, take out this if condition if doing the molecules w/o DGO
            
        # if sub_len == allnandrop_len:
        #     timeM[0] = totaltime  \
        #                + (times['DFT Geom Opt ave time'].values  \
        #                + times['MOBCAL ave time'].values)  \
        #                * sub_len

        #     comboM[0] = precombo + [np.nan, np.nan, 'nan', np.nan, 'DFT GO', sub_len]
        #     bwM[0].append(BW(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
        #     leM[0].append(LE(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
        #     saM[0].append(SA(subdf['DFT GO CCS'].values))
        # else:
        #     timeM[0] = np.nan
        #     comboM[0] = precombo + [np.nan, 'nan', np.nan, 'DFT GO', np.nan] #[bp, sp, rp, bp2, 'DFT GO', np.nan]
        #     bwM[0].append(np.nan)
        #     leM[0].append(np.nan)
        #     saM[0].append(np.nan)
...
#            print('I should leave out the full range for random, since I do not want to do MC on full range anyway.')
            ...
            for sp2 in SDS2_params:
                ...
                # # DFT GO
                # allnandrop_len = len(subdf_s2.dropna()) # I still definitely feel like there's
                #                                       # a better way to do this***
                # if sub_s2_len == allnandrop_len:            
                #     timeM[k] = totaltime_s  \
                ...
                #     comboM[k] = precombo + [bp2, sub_b2_len, 'SDS', sp2+1, 'DFT GO', sub_s2_len] #[bp, sp, rp, bp2, 'DFT GO', sub_b2_len]
                #     bwM[k].append(BW(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
                #     leM[k].append(LE(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
                #     saM[k].append(SA(subdf['DFT GO CCS'].values))
                # else:
                #     timeM[k] = np.nan
                #     comboM[k] = precombo + [bp2, sub_b2_len, 'SDS', sp2+1, 'DFT GO', np.nan] # [bp, sp, rp, bp2, 'DFT GO', np.nan]
                #     bwM[k].append(np.nan)
                #     leM[k].append(np.nan)
                #     saM[k].append(np.nan)
...
#            print('R2_params: ', R2_params)
            ...
            num_cells2 = int(2 * len(R2_params)) # here I dont need to add an extra +1 because the params have already been modified.
...
#                print('rp2', rp2)
                for j in range(MC_iter2):
                    ...
                    # # DFT GO
                    # allnandrop_len = len(subdf_r2.dropna()) # I still definitely feel like there's a 
                    #                                       #better way to do this***
                    # if sub_r2_len == allnandrop_len:            
                    ...
                    #     comboM2[k2] = precombo + [bp2, sub_b2_len, 'random', rp2, 'DFT GO', sub_r2_len] 
                    #                         #[bp, sp, rp, bp2, 'DFT GO', sub_b2_len]
                    #     bwM2[k2].append(BW(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
                    #     leM2[k2].append(LE(subdf['DFT GO CCS'].values, subdf['DFT GO Energy'].values))
                    #     saM2[k2].append(SA(subdf['DFT GO CCS'].values))
                    # else:
                    #     timeM2[k2] = np.nan
                    #     comboM2[k2] = precombo + [bp2, sub_b2_len, 'random', rp2, 'DFT GO', np.nan]
                    #     bwM2[k2].append(np.nan)
                    #     leM2[k2].append(np.nan)
                    #     saM2[k2].append(np.nan)
...
#    df = pd.DataFrame([combos, totaltimes, bw, bw_stdev, le, le_stdev, sa, sa_stdev], 
#                     index=['Methods', 'Time', 'BW', 'BW std', 'LE', 'LE std', 'SA', 'SA std']).T
#    df = df.loc[df['Methods'].str.len() != 0] # Trim the extra blank cells if any
...
if __name__ == '__main__':
    ...
    #ID = '02'
    #directory = f'/Users/niel007/Documents/excel-and-data/conformer_ccs'
    ...
    results.to_csv(join(directory, f'molid{ID}_combo_methods_results-v2.csv'), index=False)
    methods.to_csv(join(directory, f'molid{ID}_combo_methods_resultsM-v2.csv'), index=False)
